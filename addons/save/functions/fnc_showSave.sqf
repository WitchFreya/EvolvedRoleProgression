...
player setVariable [QGVARMAIN(history), _newHistory];

private _name /* string */ = name player;
private _role /* string */ = player call EFUNC(roles,unitRole);
private _roleName /* string */ = _role call EFUNC(roles,displayName);
private _opsPlayed /* number */ = [player, _role] call EFUNC(history,opCount);
